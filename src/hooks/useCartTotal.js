...
const useCartTotal = (cart, checkout) => {
  ...
  const [shipping, setShipping] = useState({
    regular: { amount: 0, provider: "" },
    express: { amount: 0, provider: "" },
  });
  ...
  const subTotals = cart.length
    ? cart.map(
        (item) => item.quantity * (item.product.price ? item.product.price : 0)
      )
    : [];
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const regularData = shipment.rates.filter((item) =>
          item.attributes.includes("BESTVALUE")
        )[0];

        const expressData = shipment.rates.filter((item) =>
          item.attributes.includes("FASTEST")
        )[0];

        setShipping({
          regular: {
            amount: parseFloat(regularData.amount),
            provider: regularData.provider,
          },
          express: {
            amount: parseFloat(expressData.amount),
            provider: expressData.provider,
          },
        });
        // console.log("Shipping: ", { regular, express });
        setTotal(
          (parseFloat(subTotal) + parseFloat(regularData.amount)).toFixed(2)
        );
